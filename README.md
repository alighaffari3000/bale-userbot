...
| وقتی بله نشست را باطل می‌کند (خروج از همهٔ دستگاه‌ها)، وب‌سوکت هنوز `101` می‌گیرد و بلافاصله با کد `4401` بسته می‌شود؛ `Client.start` این را از قطعی شبکه تشخیص نمی‌دهد | هر ۵ ثانیه با توکن مرده دوباره وصل می‌شود — بی‌پایان و بی‌صدا؛ تنها ردّش `Cannot write to closing transport` است (روی اکانت واقعی: ۳۳۹ بار در ساعت) | `GuardedSession` (نشست پیش‌فرض `KitClient`) بعد از اتصال نیم‌ثانیه گوش می‌دهد، رد شدن را با `SessionRejected` اعلام می‌کند، تلاش بعدی را از ۳۰ ثانیه تا ۱۵ دقیقه عقب می‌اندازد، و فایل نشست را قبل از هر تلاش دوباره می‌خواند تا لاگین جدید بدون ری‌استارت جا بیفتد |
