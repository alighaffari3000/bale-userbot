"""`python -m balekit <command>` — login, whoami, session info."""
...
async def cmd_whoami(config: Config) -> int:
    ...
    print(f"name:         {getattr(client.me, 'name', None)}")
    print(f"session file: {config.session_file}")
    return 0


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="balekit")
    ...
    sub.add_parser("whoami", help="show the account behind the stored session")
    ...
    return asyncio.run(cmd_whoami(config))
