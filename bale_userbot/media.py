...
#: Media prefixes that already say what a file is.
...
def detect_kind(mime_type: str | None, name: str | None = None) -> MessageKind:
    ...
    The extension is consulted whenever the MIME type does not classify the
    file by itself — not only when it is missing. Container formats are the
    reason: BaleClient's byte sniffer reports `application/ogg` for an Ogg
    file, which would otherwise be sent as a plain document.
    """
    mime = (mime_type or "").lower()
    if not mime.startswith(_MEDIA_PREFIXES):
        guessed = (mimetypes.guess_type(name or "")[0] or "").lower()
        if guessed.startswith(_MEDIA_PREFIXES):
            mime = guessed
...
async def _send_audio(
    ...
    *,
    ...
    if not any((album, genre, track)) and thumb is None:
        return await client.send_audio(audio=file, duration=duration, **common)

    # Tags or album art present: bypass send_audio, which builds an ext block
    # without the tags and takes no thumbnail at all.
...
async def resend(
    ...
    *,
    ...
    if info.kind in (MessageKind.LOCATION, MessageKind.CONTACT):
        # JSON content copies verbatim too, keeping any subfield we do not
        # model (a contact card can carry more than a name and numbers).
        from .extras import json_block_content, send_content

        return await send_content(
            client, json_block_content(message), chat_id, chat_type, reply_to=reply_to
        )

    if info.media is None:
        raise ValueError(f"message {info.message_id} carries no file to resend")

    if info.kind is MessageKind.STICKER:
        # Stickers are references into a server-side collection, not
        # documents; they carry no caption and copy over verbatim.
        from .extras import send_content, sticker_content

        return await send_content(
            client, sticker_content(message), chat_id, chat_type, reply_to=reply_to
        )
    ...
    # Sent back as-is, a plain file keeps every original field. With a
    # caption BaleClient rebuilds the DocumentMessage and drops thumb and
    # ext, so both are handed back explicitly.
    if caption is None:
        return await client.send_document(file=document, **common, use_own_content=True)
    return await client._send_file_message(
        file=document,
...
def _thumb_bytes(document: DocumentMessage) -> bytes | None:
    """The cover thumbnail, only when it is inline bytes.

    `Thumbnail.image` is `str | bytes`: the wire can carry a URL instead of
    the image, and `FileInput` reads any string as a filesystem path and
    raises. A remote thumbnail is skipped — Bale regenerates one.
    """
    thumb = document.thumb
    if thumb is None or not isinstance(thumb.image, (bytes, bytearray)):
        return None
    return bytes(thumb.image) or None
...
async def download(
    ...
    path is returned.
    """
    media, fallback = _media_of_source(source)
    ...
    if isinstance(destination, (str, Path)):
        ...
        if path.is_dir():
            path = path / suggest_filename(media, fallback)
...
def _media_of_source(
    source: Message | MessageInfo | MediaInfo,
) -> tuple[MediaInfo, str]:
    """The attachment, plus a name stem describing what it is."""
    ...
    return info.media, info.kind.value
